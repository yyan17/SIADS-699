...
agg_topic_ticker_sent_cols = ['date', 'agg_polarity', 'agg_compound', 'topic_polarity', 'topic_compound',
                              'ticker_polarity', 'ticker_compound']
...
def read_sentiment_features(data_paths, topic, ticker):
    ...
    sentiment_df = agg_sentiment_df.merge(agg_topic_sentiment_df, on='date', how='outer').merge(ticker_sentiment_df,
                                                                                                on='date', how='outer')
...
def preprocess_index_features(path, ticker, day='today'):
    ...
    ticker_df[ticker + '_' + 'PERCENT_CHANGE'] = (ticker_df['Open'] - ticker_df['yesterday_Close']) / ticker_df[
        'yesterday_Close']
    ...
    if day == 'yesterday':
        ...
        ticker_df[ticker + '_' + 'PERCENT_CHANGE'] = (ticker_df['Close'] - ticker_df['yesterday_Close']) / ticker_df[
            'yesterday_Close']
